...
### P4：函数 SQL 端到端测试
- [x] 创建 `tests/functions/` 目录，覆盖全部 123 个内置函数
- [x] 聚合函数测试（19 个）：COUNT, SUM, AVG, MIN, MAX, STDDEV, VARIANCE, CORR, PERCENTILE, COLLECT_LIST 等
- [x] 数学函数测试（18 个）：ROUND, FLOOR, CEIL, ABS, SQRT, EXP, LN, LOG10, POWER, MOD, SIGN, RAND, GREATEST, LEAST, WIDTH_BUCKET 等
- [x] 字符串函数测试（21 个）：CONCAT, SUBSTRING, UPPER, LOWER, TRIM, LENGTH, REPLACE, REVERSE, LOCATE, LPAD, RPAD, INITCAP, ASCII, SPLIT 等
- [x] 条件函数测试（5 个）：IF, COALESCE, NVL, NULLIF, CAST
- [x] 日期时间函数测试（25 个）：CURRENT_DATE, UNIX_TIMESTAMP, FROM_UNIXTIME, YEAR, MONTH, DAY, DATEDIFF, DATE_ADD, DATE_FORMAT, ADD_MONTHS, LAST_DAY, MONTHS_BETWEEN, QUARTER, TRUNC 等
- [x] 窗口函数测试（11 个）：ROW_NUMBER, RANK, DENSE_RANK, LEAD, LAG, NTILE, FIRST_VALUE, LAST_VALUE, CUME_DIST, PERCENT_RANK, NTH_VALUE
- [x] 杂项函数测试（24 个）：REGEXP_REPLACE, BASE64, HEX, MD5, SHA1, SHA2, CRC32, HASH, GET_JSON_OBJECT, CURRENT_USER, VERSION, MASK 等
